...
//        init.setAplEmail((info.getEmail(5,9)).toString());
        ...
        action.selectByText(page1.plat_goods_id(),"阳光I保C-两月缴");
...
//        driver.switchTo().frame("ifm");
        ...
        for (int i=0;i<2;i++) {
            swp1.swipe(list.get(0), 20, 30, 20, -30);
            action.sleep(1);
        }
        action.sleep(2);
        action.click(page3.quding());
        init.setAplCity1(action.getText(page3.citycont()).replaceAll("/*-", ""));
        action.click(page3.pay_20Y());
        action.click(page3.pay_type_M());
        action.click(page3.baoer30());
        action.sleep(3);
...
//        action.sleep(7);
        ...
        target.setAplCity1(action.getText(page4.citycont()).replaceAll("/*-", ""));
...
//        target.setAplEmail(action.getAttribute(page4.apleml(),"value"));
    ...
    //ibaoC 3月版
    public void ibaoC3Test() throws  Exception{
        loginpage page=new loginpage();
        homepage page1=new homepage();
        iBaoC2page page3=new  iBaoC2page();
        iBaoCpage page4=new iBaoCpage();
...
//        init.setAplEmail((info.getEmail(5,9)).toString());
        ...
        action.selectByText(page1.plat_goods_id(),"阳光I保C-三月缴");
        action.click(page1.save());
        action.sleep(6);
        //切换frame
        action.switchToFrame(page3.product_frame());
...
//        driver.switchTo().frame("ifm");
        ...
        SwipeScreenOrElement swp2 = new SwipeScreenOrElement(driver, list.get(0));
        for (int i=0;i<2;i++) {
            swp1.swipe(list.get(0), 20, 30, 20, -40);
            action.sleep(1);
        }
        action.sleep(2);
        action.click(page3.quding());
...
//        target.setAplEmail(action.getAttribute(page4.apleml(),"value"));
        ...
        /* 跳转收银台是否成功 */
        boolean paysucc=true;
        if ( action.isElementsPresent(page4.success(),10) ){
            action.successend();
            Reporter.log("跳转收银台成功  "+paysucc);
        }else {
            paysucc=false;
            Reporter.log("跳转收银台失败  "+paysucc);
        }
        //第二页信息比对
        Reporter.log("预约单填写的信息为 ： "+init.toString());
        Reporter.log("用户链接下发的信息为 ： "+target.toString());
